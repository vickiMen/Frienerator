...
const downloadFolder = "./videoLinks"  //need to decide on another folder maybe
let videoCounter = 0
...
let files = fs.readdirSync(downloadFolder) //reads through the VideoLinks folder, where the cut videos are

//filters just the mp4 videos and pushes their name only into the seperateFilesToConcat array
for (let i of files){
    if (path.extname(i) == ".mp4"){
        seperateFilesToConcat.push(path.basename(`./videoLinks/${i}`, ".mp4"))
    } 
}
